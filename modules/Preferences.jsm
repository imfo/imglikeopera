...
Preferences.prototype = {
  ...
  /**
   * Set a preference to a value, but resets a preference if its' type differs from new balue type.
   */
  ...
  _set: function(prefName, prefValue, overwrite) {
    ...
    let storedValueType = prefSvc.getPrefType(prefName);
    switch (prefType) {
      case "String":
        {
...
// Preferences of extension.
// FIXME: extension id.
var Prefs = new Preferences("extensions.imglikeopera@imfo.ru.");
...
/*==============================================================================*/
/**
 * observe2/ignore2 - функции для наблюдения за веткой настроек.
 *
 * Использование аналогично observe/ignore, за исключением того, что
 * наблюдатели получают уведомление об изменениях не конкретной настройки,
 * а всей ветки.
 */
